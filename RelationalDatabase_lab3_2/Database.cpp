...
Database::Database(DataLog dataLog) {
	...
	for (unsigned int i = 0; i < schemes.size(); i++) {
		string content = schemes[i].getIDContent();
		...
		relations.emplace(content, Relation(content, scheme));
	}
	
	vector<Predicate> facts = dataLog.getFacts();

	for (unsigned int i = 0; i < facts.size(); i++) {
		string content = facts[i].getIDContent();
		...
		relations.at(content).addTuple(tuple);
	}
